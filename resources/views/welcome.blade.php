    ...
    <body class="bg-[#FDFDFC] dark:bg-[#0a0a0a] text-[#1b1b18] min-h-screen">
        <div class="container mx-auto px-4 py-8">
            ...
            <header class="flex justify-between items-center mb-12 bg-gray-50 dark:bg-gray-900 p-4 rounded-lg shadow-sm">
                <div class="text-2xl font-extrabold bg-gradient-to-r from-blue-600 to-purple-600 bg-clip-text text-transparent hover:from-purple-600 hover:to-blue-600 transition duration-300">Md.Emon</div>
                
                @if (Route::has('login'))
                    <nav class="flex items-center gap-4">
                        @auth
                            <a href="{{ url('/dashboard') }}" class="px-5 py-2 bg-black text-white dark:bg-white dark:text-black rounded-md hover:opacity-90 transition">
                                Dashboard
                            </a>
                        @else
                            <a href="{{ route('login') }}" class="px-5 py-2 border border-gray-300 dark:border-gray-700 rounded-md hover:bg-gray-100 dark:hover:bg-gray-800 transition">
                                Log in
                            </a>

                            @if (Route::has('register'))
                                <a href="{{ route('register') }}" class="px-5 py-2 bg-black text-white dark:bg-white dark:text-black rounded-md hover:opacity-90 transition">
                                    Register
                                </a>
                            @endif
                        @endauth
            ...
            <main class="flex flex-col lg:flex-row gap-10 items-center justify-center min-h-[60vh] my-12">
                <div class="max-w-md">
                    ...
                    {{-- <div class="flex gap-4">
                        <a href="#contact" class="px-6 py-3 bg-black text-white dark:bg-white dark:text-black rounded-md hover:opacity-90 transition">
                            Contact Me
                        </a>
